...
class RadioController extends ChangeNotifier {
  ...
  // Internal list to manage state before notifying listeners
  ...
  Future<void> connect() async {
    ...
    _btStreamSubscription = _commandConnection!.input!.listen(
        _onDataReceived,
        onDone: dispose,
        onError: (e) {
  ...
  void _handleEvent(EventNotificationBody eventBody) async {
    ...
    switch (eventBody.eventType) {
      ...
      // ---- KEY CHANGE: Corrected logic for handling APRS data ----
      case EventType.DATA_RXD:
        ...
        if (fragment.isFinalFragment) {
          ...
          try {
            if (kDebugMode) {
              print("Benshi PARSING APRS PACKET (bytes): $fullPacketBytes");
            }
            final newPacket = AprsPacket.fromAX25Frame(fullPacketBytes);

            // Notify listeners about the latest packet
            latestAprsPacket.value = newPacket;

            // Update the main list of packets if it has location data
            if (newPacket.latitude != null && newPacket.longitude != null) {
              ...
              // Notify listeners by updating the ValueNotifier's value
